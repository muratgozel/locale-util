...
export class Supplement {
    ...
    async generate(supplement: any, supplementLocal: any, phoneNumberMetadata: any) {
        ...
        const englishNames = supplementLocal.filter((obj: { language: string }) => obj.language === 'en')[0]

        const languages = supplement.languageCodes.map((languageCode: string) => {
            const nativeNames = supplementLocal.some(({language}: {language: string}) => language === languageCode)
                ? supplementLocal.filter(({language}: {language: string}) => language === languageCode)[0]
                : undefined

            return {
                code: languageCode,
                nativeName: nativeNames && nativeNames.languageCodeNativeNames.some(({code}: {code: string}) => code === languageCode)
                    ? nativeNames.languageCodeNativeNames.filter(({code}: {code: string}) => code === languageCode)[0].localName
                    : '',
                englishName: englishNames.languageCodeNativeNames.some((obj2: { code: any }) => obj2.code === languageCode)
                    ? englishNames.languageCodeNativeNames.filter((obj2: { code: any }) => obj2.code === languageCode)[0].localName
                    : ''
            }
        })
        ...
        const currencies = supplement.currencies.map(({code, num}: {code: string, num: number}) => {
            const countryCode: string = Object.keys(supplement.countryCurrencyMap).filter((countryCode) => supplement.countryCurrencyMap[countryCode] === code)[0] as string
            const languageCode = Object.hasOwn(supplement.countryLanguages, countryCode) ? (supplement.countryLanguages[countryCode][0] || 'en') : 'en'
            const nativeName = supplementLocal.some(({language}: {language: string}) => language === languageCode)
                ? supplementLocal.filter(({language}: {language: string}) => language === languageCode)[0].languageCodeNativeNames.filter(({code}: {code: string}) => code === languageCode)[0].localName
                : ''

            return {
                code,
                num,
                englishName: englishNames.languageCodeNativeNames.filter(({code}: {code: string}) => code === languageCode)[0].localName,
        ...
        const countries = supplement.countryCodes.map((countryCode: string) => {
            const languageCode = Object.hasOwn(supplement.countryLanguages, countryCode) ? (supplement.countryLanguages[countryCode][0] || 'en') : 'en'
            const nativeNames = supplementLocal.some(({language}: {language: string}) => language === languageCode)
                ? supplementLocal.filter(({language}: {language: string}) => language === languageCode)[0]
                : undefined
            const nativeName = nativeNames && nativeNames.countryCodeNativeNames.some(({code}: {code: string}) => code === countryCode)
                ? nativeNames.countryCodeNativeNames.filter(({code}: {code: string}) => code === countryCode)[0].localName
                : ''
            return {
                code: countryCode,
                englishName: englishNames.countryCodeNativeNames.filter(({code}: {code: string}) => code === countryCode)[0].localName,
                nativeName: nativeName
            }
        })
        ...
        const timezones = supplement.countryCodes.reduce((memo: object[], code: string) => {
            return memo.concat(
        ...
        const countryCallingCodes = territory.reduce((memo: any, obj: any) => {
    ...
    async read() {
        ...
        const languageCodesExtended: string[] = []
        const countryCodes: string[] = []
        const countryLanguages: {[index: string]: string[]} = {}
        const countryLanguagesExtended: {[index: string]: string[]} = {}
        territoryInfo.territory.map(({languagePopulation, type}: any) => {
            if (type === 'ZZ') return

            countryCodes.push(type)

            if (isObject(languagePopulation)) languagePopulation = [languagePopulation]
            if (!languagePopulation) languagePopulation = []
            let languages = languagePopulation.filter((obj: any) => obj.officialStatus && obj.officialStatus === 'official')
            if (languages.length < 1) {
                languages = languagePopulation.filter((obj: object) => Object.hasOwn(obj, 'officialStatus'))
            }
            if (languages.length < 1) {
                languages = [languagePopulation[0]]
            }
            languages = languages.filter((obj: { type: string }) => this.reLanguageString.test(obj.type) && obj.type !== 'und')
            countryLanguages[type] = languages.map((obj: {type: string}) => {
                if (!languageCodes.includes(obj.type)) languageCodes.push(obj.type)
                return obj.type
            })

            countryLanguagesExtended[type] = languagePopulation.map((obj: {type: string}) => {
                if (!languageCodesExtended.includes(obj.type)) languageCodesExtended.push(obj.type)
                return obj.type
            })
        })

        const currencies: {code: string, num: number}[] = []
        const currencyCodes: string[] = []
        const countryCurrencyMap: {[index: string]: string} = {}
        currencyData.region.map(({iso3166, currency}: {iso3166: string, currency: any}) => {
        ...
        codeMappings.currencyCodes.map(({type, numeric}: {type: string, numeric: string}) => {
    ...
    async readCountries({languageCodes}: {languageCodes: string[]}) {
        const jobs = (await readdir(this.pathSourceMain))
            ...
            .map(async (resource) => {
                ...
                if (isObject(localeDisplayNames.languages.language)) localeDisplayNames.languages.language = [localeDisplayNames.languages.language]

                const languageCodeNativeNames = localeDisplayNames.languages.language.map((obj: { [x: string]: any, type: any }) => ({code: obj.type, localName: obj['#text']}))
                const countryCodeNativeNames = localeDisplayNames.territories.territory.map((obj: { [x: string]: any; type: any }) => ({code: obj.type, localName: obj['#text']}))
                const letters = characters.exemplarCharacters[0].slice(1, -1).split(' ')
                const currencies = numbers.currencies && Array.isArray(numbers.currencies.currency) ? numbers.currencies.currency.map((obj: { type: string, displayName: any[] }) => ({code: obj.type, localName: Array.isArray(obj.displayName) ? obj.displayName[0] : obj.displayName})) : []
